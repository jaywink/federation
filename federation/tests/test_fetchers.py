...
class TestRetrieveRemoteContent:
    ...
    async def test_calls_activitypub_retrieve_and_parse_content(self, mock_import):
        mock_retrieve = AsyncMock()
        mock_import.return_value = mock_retrieve
        await retrieve_remote_content("https://example.com/foobar")
    ...
    @patch("federation.fetchers.importlib.import_module")
    async def test_calls_diaspora_retrieve_and_parse_content(self, mock_import):
        mock_retrieve = AsyncMock()
        mock_import.return_value = mock_retrieve
        await retrieve_remote_content("1234", handle="user@example.com", entity_type="post", sender_key_fetcher=sum)
        mock_retrieve.retrieve_and_parse_content.assert_called_once_with(
...
class TestRetrieveRemoteProfile:
    ...
    async def test_retrieve_remote_profile__url_calls_activitypub_retrieve(self, mock_identify, mock_validate, mock_import):
        mock_utils = AsyncMock()
        mock_import.return_value = mock_utils
        await retrieve_remote_profile("https://example.com/foo")
    ...
    async def test_retrieve_remote_profile__handle_calls_both_activitypub_and_diaspora_retrieve(
            self, mock_identify, mock_validate, mock_import,
    ):
        mock_utils = AsyncMock(retrieve_and_parse_profile=AsyncMock(return_value=None))
        mock_import.return_value = mock_utils
        await retrieve_remote_profile("user@example.com")
        calls = [
